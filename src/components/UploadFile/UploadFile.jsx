...
import { Upload, Popover, Spin, message} from 'antd';
...
const UploadFile = (props) => {
  ...
  const uploadProps = {
    ...
    onRemove,
    beforeUpload,
    disabled: !indexInfo || uploading || analysising,
  };
  ...
  return (
    <div>
      ...
      <Dragger {...uploadProps}>
        {
          (uploading || analysising) ? (
            <div className={styles.loading}>
              <Spin size="smail" />
              <div className={styles.loadingText}>{uploading ? 'Uploading' : 'Analysising'}</div>
            </div>
          ) : (
            <>
